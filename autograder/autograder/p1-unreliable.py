...
def get_unreliable_response( move, chance, k ):
    ...
    if np.random.random() < chance and reds != k and reds!=k-1:
        reds += 1
    return reds, max(whites_and_reds-reds,0)
n_list = [5,8,10,12,12]
k_list = [3,4,5,6,7]
...
for t in range(len(n_list)):
    ...
    while red_actual < k and tot_moves < moves_limit:
        ...
        # previous list.append(move) was changing all elements to appended element, link: https://stackoverflow.com/questions/5280799/list-append-changing-all-elements-to-the-appended-item
        guess_list.append(move[:])
        # print(guess_list," ", move)
